...
export default function PersonCard({ person }) {
  ...
  const handleColorChange = (newColor) => {
    ...
    if (existing.includes(newColor.toLowerCase())) {
      finalColor = shiftHue(newColor);
    }

    updatePerson(person.id, { color: finalColor });
  };
...
<div>
  ...
  <div className="mt-3 space-y-4">
    ...
    {/* Custom Colour → uses fallbackColor */}
    <div>
      <div className="text-xs font-medium text-gray-600 mb-1">
  ...
  type="button"
  onClick={() => {
    // Set the colour immediately to fallbackColor
  ...
  className={`
    w-8 h-8 rounded-full border shadow transition-transform
    ${person.color === fallbackColor ? "ring-2 ring-blue-600 scale-110" : ""}
  `}
